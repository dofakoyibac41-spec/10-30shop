...
**Задачи:**
- [x] Страница `/admin/login` — форма логин + пароль
- [x] `useAuth.js` composable — `login()`, `logout()`, `getToken()`, `getAuthHeader()` через localStorage
- [x] Логика авторизации: POST `/api/auth/login` → получить JWT → сохранить через `useAuth.js`
- [x] **[РЕК-3 из ревью Sprint 1 — выполнено]** Route guard во Vue Router — `router.beforeEach`: `/admin` без токена → редирект на `/admin/login`
- [x] `AdminPage.vue` — две вкладки: «Категории» и «Товары»
- [x] Форма создания категории: название + URL обложки (необязательно) + превью
- [x] Список категорий с удалением (запрет если в ней есть товары → 409)
- [x] **[РЕК-1 из ревью Sprint 1 — выполнено]** `PATCH /api/categories/:id` на бэкенде + inline-редактирование в UI
- [x] Форма добавления товара (название, описание, категория, картинка)
- [x] Выпадающий список категорий в форме товара (из API)
- [x] Поле URL картинки — валидация что URL не пустой + `required`
- [x] Превью картинки по URL прямо в форме (`<img>` с `@error`-очисткой)
- [x] Список товаров с чекбоксом у каждого товара
- [x] Чекбокс «Выбрать все» + `:indeterminate` при частичном выборе
- [x] Кнопка «Удалить выбранные (N)» — активна только если что-то выбрано
- [x] `DELETE /api/products/bulk` — принимает массив `ids`, удаляет все сразу
- [x] Одиночное удаление товара (кнопка у каждой строки)
- [x] Кнопка «Выйти» — очищает токен, редирект на `/admin/login`
- [x] Toast-уведомления об успехе/ошибке через `<Transition>` (3 сек, без `alert()`)
- [x] `useAdminApi()` — 6 защищённых методов в `useApi.js` (все с JWT-заголовком)

**Ревью:** REVIEW_REPORT_v1 — 0 критических, 4 рекомендации. REVIEW_REPORT_v2 — все 4 исправлены:
  - `import` перенесён в начало `useApi.js`
  - Два `import` из одного модуля объединены в `AdminPage.vue`
  - Устаревший комментарий «stub» обновлён в `useAuth.js`
  - 3 инлайн `style=` заменены на CSS-классы `.divider--tabs` / `.divider--section`

**Результат:** Можно добавлять, редактировать и удалять товары/категории через UI. Вход защищён JWT.
...
| Sprint 4 | Админка | ✅ Готово |
| Sprint 5 | Деплой на Timeweb | ~30 мин |
| **Итого** | | **~30 мин осталось** |
